...
public final class Base64 {
        public static String toString( byte[] data )
        {
                ...
                if ( binaryChar == null ) return null;
        ...
        public static byte[] toByte( String data )
        {
                ...
                for ( ; k < j - 1; k++ )
                {
                        if ( !isData( c = ac[i1++] ) || !isData( c1 = ac[i1++] ) || !isData( c2 = ac[i1++] ) || !isData( c3 = ac[i1++] ) ) return null;
                ...
                if ( !isData( c = ac[i1++] ) || !isData( c1 = ac[i1++] ) ) return null;
                byte0 = BASE64_ALPHABET_BYTE[c];
                byte1 = BASE64_ALPHABET_BYTE[c1];
                c2 = ac[i1++];
                c3 = ac[i1++];
                ...
                {
                        if ( isPad( c2 ) && isPad( c3 ) ) if ( (byte1 & 0xf) != 0 )
                        {
                                return null;
                        }
                        else
        ...
        private static int removeWhiteSpace( char ac[] )
        {
                ...
                int j = ac.length;
                for ( int k = 0; k < j; k++ )
                        if ( !isWhiteSpace( ac[k] ) ) ac[i++] = ac[k];
                return i;
        }
        ...
        private static char[] convertCharToByte( byte binary[] )
        {
                ...
                if ( binaryBitLength == 0 ) return new char[0];
